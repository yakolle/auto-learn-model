...
class GBTClassificationModel private[ml](
  ...
  private var minTrainScore = Double.MinValue
  private var maxTrainScore = Double.MaxValue

  override protected def transformImpl(dataset: Dataset[_]): DataFrame = {
    val bcastModel = dataset.sparkSession.sparkContext.broadcast(this)
    val scoreUDF = udf { (features: Any) =>
      bcastModel.value.predict(features.asInstanceOf[Vector])
    }
    val predictUDF = udf { (score: Double) =>
      if (score > 0.0) 1.0 else 0.0
    }
    val scores = dataset.withColumn("score", scoreUDF(col($(featuresCol)))).withColumn($(predictionCol), predictUDF(col("score")))

    val minMaxScore = scores.agg(min("score"), max("score")).head
    minTrainScore = if (Double.MinValue == minTrainScore) minMaxScore.getDouble(0) else minTrainScore
    maxTrainScore = if (Double.MaxValue == maxTrainScore) minMaxScore.getDouble(1) else maxTrainScore

    val probabilityUDF = udf { (score: Double) =>
      val prob = if (score > maxTrainScore) 1.0
      else if (score >= 0.0) 0.5 * score / maxTrainScore + 0.5
      else if (score >= minTrainScore) 0.5 * (minTrainScore - score) / minTrainScore
      else 0.0
      Vectors.dense(Array(1.0 - prob, prob))
    }
    scores.withColumn("probability", probabilityUDF(col("score")))
  }
  ...
  override def toDebugString: String = {
    minTrainScore.toString + "," + maxTrainScore.toString + "\n" + super.toDebugString
  }
